...
#import "UIColor+SPSDKCommon.h"

@implementation UIColor (SPSDKCommon)

+ (instancetype)colorForHex:(NSInteger)hex
{
    NSUInteger a = ((hex >> 24) & 0x000000FF);
    float fa = ((0 == a) ? 1.0f : (a * 1.0f) / 255.0f);
    
    return [UIColor colorForHex:hex alpha:fa];
}

+ (instancetype)colorForHex:(NSInteger)hex alpha:(CGFloat)alpha
{
...
+ (instancetype)fromShortHexValue:(NSUInteger)hex alpha:(CGFloat)alpha
{
